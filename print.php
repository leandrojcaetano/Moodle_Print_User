...
//  Lists all the users within a given course
    ...
    $PAGE->set_url('/user/print.php', array(
            'page' => $page,
            'perpage' => $perpage,
            'mode' => $mode,
    ...
    $rolenamesurl = new moodle_url("$CFG->wwwroot/user/print.php?contextid=$context->id&sifirst=&silast=");
    ...
    add_to_log($course->id, 'user', 'view all', 'print.php?id='.$course->id, '');
...
/// and if so, set $currentgroup to reflect the current group
	...
	$PAGE->set_title("$course->shortname: Imprimir participantes");
    ...
    $baseurl = new moodle_url('/user/print.php', array(
...
/*    if ($mycourses = enrol_get_my_courses()) {
        $courselist = array();
        $popupurl = new moodle_url('/user/print.php?roleid='.$roleid.'&sifirst=&silast=');
...
*/
            if ($matchcount > 0) {
                ...
                foreach ($userlist as $user) {
                    ...
                    if ($CFG->bloglevel > 0) {
                        $links[] = html_writer::link(new moodle_url('/blog/print.php?userid='.$user->id), get_string('blogs','blog'));
                    }

                    if (!empty($CFG->enablenotes) and (has_capability('moodle/notes:manage', $context) || has_capability('moodle/notes:view', $context))) {
                        $links[] = html_writer::link(new moodle_url('/notes/print.php?course=' . $course->id. '&user='.$user->id), get_string('notes','notes'));
                    }
    ...
    /* retirando os botoes abaixo da tabela de participantes
	if ($bulkoperations) {
    ...
    if (has_capability('moodle/site:viewparticipants', $context) && $totalcount > ($perpage*3)) {
        echo '<form action="print.php" class="searchform"><div><input type="hidden" name="id" value="'.$course->id.'" />';
